<x-guest-layout>
    <x-authentication-card>
        ...
        <x-slot name="footer">
            <span class="mt-8 text-sm text-neutral-600 dark:text-neutral-400" href="{{ route('login') }}">
                {{ __('Existing user?') }}
                <a href="{{ route('login') }}" class="font-bold text-accent-800 dark:text-accent-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent-500 dark:focus:ring-offset-neutral-800">
                    {{ __('Login') }}
                </a>
            </span>
        </x-slot>
